...
use crate::automation::recovery::{RecoveryDecision, RecoveryEngine};
use crate::automation::state::{
    AutomationEvent, AutomationState, AutomationStatus, EventEntry, SideEffectClass, StepStatus,
};
...
impl Orchestrator {
    pub fn run(
        ...
        if !state
            .event_stream
            .iter()
            .any(|e| matches!(e.event, AutomationEvent::TaskStarted))
        {
            let timestamp_ms = std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap_or_default()
                .as_millis();
            state.event_stream.push(EventEntry {
                timestamp_ms,
                event: AutomationEvent::TaskStarted,
            });
        }
        ...
        loop {
            ...
            match state.status {
                ...
                AutomationStatus::Executing => {
                    ...
                    state.event_stream.push(EventEntry {
                        timestamp_ms,
                        event: AutomationEvent::StepStarted {
                    ...
                    if step.side_effect_class == SideEffectClass::HighRiskHumanConfirm {
                        ...
                        if !confirmed {
                            let err = AutomationError::Policy(
                                "High-risk action requires user confirmation token (Blocked before execution)"
                                    .to_string(),
                            );
                            ...
                            state.event_stream.push(EventEntry {
                                timestamp_ms,
                                event: AutomationEvent::ErrorOccurred {
                            ...
                            state.event_stream.push(EventEntry {
                                timestamp_ms,
                                event: AutomationEvent::UserAcknowledgment {
                                    step_id: step.id.clone(),
                                    token: "APPROVED_BY_USER".to_string(),
                                },
                            });
                        }
                    }
                    ...
                    state.event_stream.push(EventEntry {
                        timestamp_ms,
                        event: AutomationEvent::StepExecuted {
                            step_id: step.id.clone(),
                        },
                    });
                    ...
                    if !matches!(step.side_effect_class, SideEffectClass::PureRead) {
                        ...
                        state.event_stream.push(EventEntry {
                            timestamp_ms,
                            event: AutomationEvent::SideEffectOccurred {
                                step_id: step.id.clone(),
                                class: step.side_effect_class.clone(),
                            },
                        });
                    }
                    ...
                    if let Some(verifier_id) = &step.verifier {
                        if let Some(verifier) = verifier_resolver(verifier_id) {
                            match verifier.verify(
                                &step.title,
                                &step.inputs,
                                &state.global_context.get("goal").cloned().unwrap_or_default(),
                            ) {
                                Ok(report) => {
                                    ...
                                    state.event_stream.push(EventEntry {
                                        timestamp_ms,
                                        event: AutomationEvent::StepVerified {
                                            step_id: step.id.clone(),
                                    ...
                                    if !report.success {
                                        ...
                                        state
                                            .error_history
                                            .push(AutomationError::Verification(report.message.clone()));
                                    } else {
                                ...
                                Err(e) => {
                                    success = false;
                                    error!(
                                        "Verifier '{}' execution failed for step [{}]: {}",
                                        verifier_id, step.id, e
                                    );
                            ...
                            let err = AutomationError::Policy(format!(
                                "Verifier '{}' not found for step '{}'",
                                verifier_id, step.id
                            ));
                ...
                AutomationStatus::Recovering => {
                    ...
                    let last_error = failed_step
                        .error
                        .clone()
                        .unwrap_or(AutomationError::Execution("Step failed verification".to_string()));
...

                    {
                        ...
                        state.event_stream.push(EventEntry {
                            timestamp_ms,
                            event: AutomationEvent::RecoveryDecision {
                    ...
                    match decision {
                        RecoveryDecision::RetryStep => {
                            state.error_history.push(AutomationError::Transient(format!(
                                "Transient error on step '{}', triggering simple retry",
                                failed_step.id
                            )));
                            state.status = AutomationStatus::Executing;
                        }
                        RecoveryDecision::Replan | RecoveryDecision::RollbackAndReplan => {
                            if decision == RecoveryDecision::RollbackAndReplan {
                                ...
                                if let Some(contract) = &failed_step.rollback_contract {
                                    info!(
                                        "Executing rollback contract '{}' for step [{}]",
                                        contract.strategy, failed_step.id
                                    );

                                    // [Phase 1.95] Record decision before action
                                    state.error_history.push(AutomationError::Execution(format!(
                                        "Replan required, triggering rollback ({}) for step '{}'",
                                        contract.strategy, failed_step.id
                                    )));

                                    executor
                                        .rollback(failed_step, &state.global_context)
                                        .map_err(|e| anyhow!("Rollback failed for step '{}': {}", failed_step.id, e))?;
                                } else {
                                    warn!("Step '{}' requires rollback but no contract found — proceeding with replan anyway", failed_step.id);
                                    state.error_history.push(AutomationError::Execution(format!(
                                        "Replan triggered WITHOUT rollback (missing contract) for step '{}'",
                                        failed_step.id
                                    )));
                                }
                                ...
                                // Record plain replan
                                state.error_history.push(AutomationError::Execution(
                                    "Verification failed, triggering re-plan".to_string(),
                                ));
                            }
                            ...
                            if state.replan_count > state.max_replan_count {
                                state.status = AutomationStatus::Failed;
                                return Err(anyhow!(
                                    "Max replan count ({}) reached, aborting to prevent infinite loops.",
                                    state.max_replan_count
                                ));
                            }
                            ...
                            info!(
                                "Re-planning required: plan_version={}, replan_count={}/{}",
                                state.plan_version, state.replan_count, state.max_replan_count
                            );
                            ...
                            if state.current_step_index < state.steps.len() {
                                ...
                                let last_error = failed_step_ref.error.clone().unwrap_or(
                                    AutomationError::Verification("Unknown verification failure".to_string()),
                                );
                        ...
                        RecoveryDecision::RollbackAndRetry => {
                            ...
                            if let Some(contract) = &failed_step.rollback_contract {
                                info!(
                                    "Executing rollback+retry contract '{}' for step [{}]",
                                    contract.strategy, failed_step.id
                                );

                                // [Phase 1.95] Record decision
                                state.error_history.push(AutomationError::Execution(format!(
                                    "Retry required with cleanup, triggering rollback ({}) for step '{}'",
                                    contract.strategy, failed_step.id
                                )));
                                ...
                                warn!(
                                    "Step '{}' requires rollback but no contract found — retrying directly",
                                    failed_step.id
                                );
                                state.error_history.push(AutomationError::Execution(format!(
                                    "Retry triggered WITHOUT rollback (missing contract) for step '{}'",
                                    failed_step.id
                                )));
                            }
                            state.dirty_state = false;
                ...
                AutomationStatus::Completed | AutomationStatus::Failed | AutomationStatus::Paused => {
                    ...
                    if state.status == AutomationStatus::Completed {
                        state.event_stream.push(EventEntry {
                            timestamp_ms,
                            event: AutomationEvent::TaskCompleted,
                        });
                        ...
                        state.event_stream.push(EventEntry {
                            timestamp_ms,
                            event: AutomationEvent::TaskFailed { reason },
                        });
                    }
                    break;
                }
