use aion_router::automation::{
    ...
    planner::Planner,
    state::{AutomationState, AutomationStatus, AutomationStep, SideEffectClass, StepStatus},
    verifier::{CoreVerifiers, Verifier},
};
...
impl Executor for MockExecutor {
    fn execute(
        ...
        if step.capability == "code_generate" {
            ...
            if step.side_effect_class == SideEffectClass::LocalWriteReversible && target_file.exists() {
    ...
    fn rollback(
        ...
        if step.capability == "code_generate" {
            ...
            if backup_file.exists() {
                ...
                println!(
                    "⏪ MockExecutor: No backup found, deleting faulty file {:?}",
                    target_file
                );
...
"#;
    ...
    state
        .global_context
        .insert("test_dir".to_string(), json!(test_dir_str.clone()));
    ...
    let verifier_resolver =
        move |name: &str| -> Option<Box<dyn Verifier>> { CoreVerifiers::resolve(name, working_dir.clone()) };
    ...
    assert_eq!(
        ...
        StepStatus::Failed,
        "Initial step should be marked Failed"
    );
    assert_eq!(
        final_state.steps[1].status,
        StepStatus::Completed,
        "Patch step should be marked Completed"
    );
...
fn test_event_stream_recording() -> anyhow::Result<()> {
    ...
    let has_step_started = events
        .iter()
        .any(|e| matches!(e, AutomationEvent::StepStarted { step_id } if step_id == "evt_step_1"));
    let has_step_executed = events
        .iter()
        .any(|e| matches!(e, AutomationEvent::StepExecuted { step_id } if step_id == "evt_step_1"));
