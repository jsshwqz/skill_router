...
impl DiscoveryRadar {
    ...
    pub fn cascade_search(query: &str, paths: &RouterPaths) -> Result<DiscoveryResult> {
        ...
        match Self::search_google(query) {
        ...
        match Self::search_http_fallback(query) {
            Ok(hits) if !hits.is_empty() => {
        ...
        match Self::search_local(query, paths) {
            Ok(hits) if !hits.is_empty() => {
    ...
    fn search_google(query: &str) -> Result<Vec<SearchHit>> {
    ...
    fn search_http_fallback(query: &str) -> Result<Vec<SearchHit>> {
    ...
    fn search_local(query: &str, paths: &RouterPaths) -> Result<Vec<SearchHit>> {
    ...
    pub fn filter_noise(hits: Vec<SearchHit>) -> Vec<SearchHit> {
        hits.into_iter()
            .filter(|hit| {
                ...
                // Remove very short snippets (likely ads or noise)
