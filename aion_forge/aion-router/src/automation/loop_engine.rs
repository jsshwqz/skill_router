            ...
            match state.status {
                AutomationStatus::Planning => {
                    ...
                    if state.steps.is_empty() {
                        ...
                        println!("🧠 Generating initial plan for goal: {}", goal_str);
                ...
                AutomationStatus::Verifying => {
                    ...
                    println!("🔍 Verifying step: {}", step.title);
                        ...
                        if let Some(verifier) = verifier_resolver(verifier_id) {
                            match verifier.verify(&step.title, &step.inputs, &state.global_context.get("goal").cloned().unwrap_or_default()) {
                                Ok(report) => {
                                    ...
                                    if !report.success {
                                        success = false;
                                        println!("❌ Verification failed: {}", report.message);
                                ...
                                Err(e) => {
                                    success = false;
                                    println!("❌ Verifier {} execution failed: {}", verifier_id, e);
                            ...
                            println!("⚠️ Verifier not found: {}", verifier_id);
                    ...
                    match decision {
                        ...
                        RecoveryDecision::Replan | RecoveryDecision::RollbackAndReplan => {
                            if decision == RecoveryDecision::RollbackAndReplan {
                                ...
                                if let Some(contract) = &failed_step.rollback_contract {
                                    println!("⏪ Executing rollback contract ({}) for step: {}", contract.strategy, failed_step.id);
                                    ...
                                    println!("⚠️ Warning: Step '{}' requires rollback but no contract found. Proceeding with Replan anyway.", failed_step.id);
                            ...
                            println!("🔄 Re-planning (v{}) required... (replan count: {}/{})", state.plan_version, state.replan_count, state.max_replan_count);
                                ...
                                match planner.generate_patch(&state, failed_step_ref, &last_error) {
                                    Ok(patch_steps) => {
                                        for fix_step in patch_steps.into_iter().rev() {
                                            println!("💉 Injecting patch step: {}", fix_step.id);
                        ...
                        RecoveryDecision::RollbackAndRetry => {
                            ...
                            if let Some(contract) = &failed_step.rollback_contract {
                                ...
                                println!("⚠️ Warning: Step '{}' requires rollback but no contract found. Retrying directly.", failed_step.id);
