...
use crate::agent_message::DelegationHop;
...
pub enum RecoveryStrategy {
    ...
    Fallback(String),
    /// 委派给备份 Agent（多 Agent 场景下的故障转移）
    ///
    /// 当 `preferred` Agent 失败时，按顺序尝试 `fallback_agents` 列表中的 Agent
    AgentFailover {
        /// 首选的备份 Agent ID
        preferred: String,
        /// 按优先级排序的后备 Agent ID 列表
...
pub struct AiNativePayload {
    ...
    /// 任务委派链（记录任务在 Agent 间的流转路径）
    ///
    /// 每次委派都追加一条 `DelegationHop`，用于：
    /// - 防止循环委派（检测 `delegation_chain` 长度超限）
    /// - 分布式追踪（对应 OpenTelemetry span 层级）
    #[serde(default)]
    pub delegation_chain: Vec<DelegationHop>,

    /// 结果回报目标（Agent ID 或 channel name）
    ///
    /// None = 同步返回给调用方
    /// Some("agent_id") = 异步发送给指定 Agent
    #[serde(default)]
    pub reply_to: Option<String>,

    /// 上游步骤的执行结果（串行管道模式中注入）
    ///
    /// 在 `task_pipeline` 执行模式下，前一步的输出会注入到此字段，
    /// 供当前步骤的 parameters 使用（类似管道 stdin）
    #[serde(default)]
    pub upstream_result: Option<Value>,
}
...
impl AiNativePayload {
    ...
    /// Builder: 指定目标 Agent（跳过自动路由）
    pub fn with_target_agent(mut self, agent_id: &str) -> Self {
        self.target_agent_id = Some(agent_id.to_string());
        self
    }

    /// Builder: 添加一个委派跳转记录
    pub fn with_delegation_hop(mut self, from: &str, to: &str, reason: &str) -> Self {
        self.delegation_chain.push(DelegationHop::new(from, to, reason));
        self
    }

    /// Builder: 设置结果回报目标
    pub fn with_reply_to(mut self, agent_id: &str) -> Self {
        self.reply_to = Some(agent_id.to_string());
        self
    }

    /// Builder: 注入上游执行结果（管道模式）
    pub fn with_upstream_result(mut self, result: Value) -> Self {
        self.upstream_result = Some(result);
        self
    }

    /// 检查委派链深度是否超过限制
    pub fn delegation_depth_exceeded(&self, max_depth: usize) -> bool {
        self.delegation_chain.len() >= max_depth
    }
