...
impl SecurityAnalyzer {
    ...
    fn analyze_file_content(path: &Path) -> Result<()> {
        ...
        if content.contains("rm -rf") || content.contains("std::fs::remove_dir_all(\"/\")") {
            bail!(
                "SECURITY VIOLATION: Malicious file deletion detected in {:?}",
                path
            );
        }

        // 规则 2: 严禁静默修改系统安全配置
        if content.contains("/etc/passwd")
            || content.contains("hosts")
            || content.contains("proxy_settings")
        {
            bail!(
                "SECURITY VIOLATION: Suspicious system file access detected in {:?}",
                path
            );
        }
        ...
        if content.contains("curl -s") || content.contains("wget -O-") {
            bail!(
                "SECURITY VIOLATION: Stealth download-and-execute pattern detected in {:?}",
                path
            );
        }
