...
fn hash_task(t: &str) -> u64 {
    ...
    t.to_ascii_lowercase().trim().hash(&mut h);
    h.finish()
}
...
impl Planner {
    ...
    pub async fn infer_capability(task: &str, registry: &CapabilityRegistry) -> Result<Option<String>> {
        ...
        let r = Self::infer_via_ai(task, registry)
            .await
            .ok()
            .flatten()
            .or_else(|| Self::infer_via_keywords(task, registry));
        if let Some(ref cap) = r {
    ...
    pub async fn infer_capability_with_paths(
        task: &str,
        registry: &mut CapabilityRegistry,
        paths: &aion_types::types::RouterPaths,
        ...
        let r = Self::infer_via_ai(task, registry)
            ...
            .or_else(|| Self::infer_via_keywords(task, registry));
        let r = if r.is_none() {
            Self::ai_discover(task, registry, &paths.capabilities_dir).await
        } else {
            r
        };
        if let Some(ref cap) = r {
    ...
    async fn ai_discover(task: &str, registry: &mut CapabilityRegistry, cap_dir: &std::path::Path) -> Option<String> {
        ...
        if n.is_empty() || registry.validate_name(&n).is_err() {
            return None;
        }
    ...
    pub async fn infer_via_ai_with_defs(
        task: &str,
        defs: &[aion_types::capability_registry::CapabilityDefinition],
    ) -> Option<String> {
        let caps: Vec<String> = defs
            .iter()
            .map(|d| format!("- {} : {}", d.name, d.description))
            .collect();
        let sys = format!("You are a capability classifier. First, understand the user's core intent. Then match it to the most relevant capability. Return ONLY the best matching capability name from this list, or 'none'.\n\nCapabilities:\n{}", caps.join("\n"));
    ...
    pub fn ai_discover_sync(
        _task: &str,
        _registry: &mut CapabilityRegistry,
        _cap_dir: &std::path::Path,
    ) -> Option<String> {
    ...
    fn infer_via_keywords(task: &str, registry: &CapabilityRegistry) -> Option<String> {
        ...
        let rules: &[(&[&str], &str)] = &[
            ...
            (&["json"], "json_parse"),
            (&["toml"], "toml_parse"),
            (&["csv", "spreadsheet", "excel"], "csv_parse"),
            (&["pdf"], "pdf_parse"),
            (&["markdown", ".md"], "markdown_render"),
            (&["image", "photo", "png", "jpg"], "image_describe"),
            ...
            (
                &["rag query", "knowledge query", "知识检索", "ask knowledge"],
                "rag_query",
            ),
            (&["rag status", "knowledge status", "知识库状态"], "rag_status"),
            (&["mcp", "mcp call", "mcp tool"], "mcp_call"),
            (
                &["路由", "route_task", "任务路由", "选引擎", "route task", "选择引擎"],
                "route_task",
            ),
            // 多模型编排
        ...
        registry
            .definitions()
            .find(|d| {
                n.contains(&d.name.replace('_', " "))
                    || d.description
                        .split_whitespace()
                        .any(|w| w.len() > 4 && n.contains(&w.to_ascii_lowercase()))
            })
            .map(|d| d.name.clone())
    }
}
