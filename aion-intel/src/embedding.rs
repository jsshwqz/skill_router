...
impl OpenAIEmbeddingProvider {
    ...
    pub async fn embed_async(&self, text: &str) -> Result<Vec<f32>> {
        ...
        let resp: Value = client.post(&format!("{}/embeddings", self.base_url))
...
impl OllamaEmbeddingProvider {
    ...
    pub async fn embed_async(&self, text: &str) -> Result<Vec<f32>> {
        ...
        let resp: Value = client.post(&format!("{}/api/embeddings", self.base_url))
            .header("Content-Type", "application/json")
            .json(&body)
            .send()
            .await?
...
pub fn create_embedding_provider() -> Box<dyn EmbeddingProvider> {
    if let (Ok(base_url), Ok(api_key)) = (
        std::env::var("AI_EMBEDDING_BASE_URL"),
