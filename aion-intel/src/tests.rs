...
mod tests {
    ...
    mod planner_tests {
        ...
        #[test]
        fn test_planner_keywords_yaml() {
            let reg = CapabilityRegistry::builtin();
            let result = Planner::infer_via_keywords_only("parse this yaml file", &reg);
            assert_eq!(result, Some("yaml_parse".to_string()));
        }

        #[test]
        fn test_planner_keywords_search() {
            let reg = CapabilityRegistry::builtin();
            let result = Planner::infer_via_keywords_only("search the web for Rust tutorials", &reg);
            assert_eq!(result, Some("web_search".to_string()));
        }

        #[test]
        fn test_planner_keywords_summarize() {
            let reg = CapabilityRegistry::builtin();
            let result = Planner::infer_via_keywords_only("summarize this long document", &reg);
            assert_eq!(result, Some("text_summarize".to_string()));
        }

        #[test]
        fn test_planner_keywords_no_match() {
            let reg = CapabilityRegistry::builtin();
            let result = Planner::infer_via_keywords_only("fly to the moon and back", &reg);
            assert!(result.is_none(), "Should not match any keyword: {:?}", result);
        }
    }
}
