...
- 用户明确本轮只做审查，不修复上述 clippy 诊断，也不扩大源码修改范围。
- 14 项 clippy 错误与 1 项 deprecated warning 作为已知静态质量问题保留；它们不改变 `cargo check --workspace` 已通过的事实，也不能被描述为严格 clippy 已通过。
- 本轮审查结论以当前证据为止，不进入阶段 0D，不修改任何 Rust 源码。
