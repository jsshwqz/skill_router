...
COPY aion-forge-cli/Cargo.toml aion-forge-cli/Cargo.toml
COPY aion-server/Cargo.toml aion-server/Cargo.toml

# 创建 dummy src 触发依赖编译缓存
RUN mkdir -p aion-types/src aion-memory/src aion-intel/src aion-router/src aion-forge-cli/src aion-server/src && \
    ...
    echo "fn main() {}" > aion-forge-cli/src/main.rs && \
...
RUN touch aion-types/src/lib.rs aion-memory/src/lib.rs aion-intel/src/lib.rs \
    aion-router/src/lib.rs aion-forge-cli/src/main.rs aion-server/src/main.rs && \
    cargo build --release --target x86_64-unknown-linux-musl

# ── Stage 2: Runtime ──────────────────────────────────────────────────────────
FROM gcr.io/distroless/static-debian12:nonroot

COPY --from=builder /build/target/x86_64-unknown-linux-musl/release/aion-forge-cli /usr/local/bin/aion-forge-cli
